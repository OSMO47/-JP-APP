...
export default function QuizPage() {
  ...
  const createQuestion = useCallback(
    (targetLevel) => {
      ...
      const distractorPool = pool.filter((item) => item !== correctWord && item.meaning !== correctWord.meaning);
      ...
      const choices = shuffle([...shuffledDistractors, correctWord]).map((item) => ({
        ...
        isCorrect: item === correctWord
      }));

      setQuestion({ ...correctWord, level: targetLevel });
      setOptions(choices);
            ...
            {question ? (
              <>
                <WordCard
                  ...
                  meaning="เลือกความหมายที่ถูกต้อง"
                ...
                {selectedIndex !== null && (
                  <div className="rounded-xl border border-moss/20 bg-white p-4 text-sm text-midnight/80">
                    {options[selectedIndex]?.isCorrect ? 'เยี่ยมมาก! คุณตอบถูกต้อง 🎉' : 'ยังไม่ถูก ลองสุ่มคำถัดไปได้เลยนะ'}
                  </div>
                )}
              </>
            ) : (
